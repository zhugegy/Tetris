  ...
  int nCoordY, unsigned char TetrisSpaceTemplate[][TETRIS_PLAY_SPACE_X])
{
  ...
  //（已过期注释）如果可以消除行，那么每消一行pulishment -100，消行优先
  //20170101如果是长条的话，能消行就直接消行（nValue == 11 的判断不严谨，但暂时能用）
  //if (pstParam->pstFirstBlockElementCOM->nValue == 11)
  //20170101 已修改为 nSerial == 1
  //从效率更高的角度考虑，此判断可以前移，但目前这样没问题
  ...
  int nCoordY, unsigned char TetrisSpaceTemplate[][TETRIS_PLAY_SPACE_X])
{
  ...
  //20170101如果可以消行，而且是长条的话，直接消行（nValue == 11 的判断不严谨，但暂时能用）
  //if (pstParam->pstFirstBlockElementCOM->nValue == 11)
  //20170101 已修改为 nSerial == 1
  if (pstParam->pstFirstBlockElementCOM->nSerial == 1)
  {
